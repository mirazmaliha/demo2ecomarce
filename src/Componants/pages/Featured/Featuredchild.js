   ...
   <Link className='cursor-pointer' to={`/product/${sho.id}`}>
    ...
    <div className='absolute pointer-events-none flex justify-center items-end inset-0 z-50'>
      <div className='w-full flex opacity-0 translate-y-6 transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:translate-y-0'>
        <button onClick={()=> handleCart(sho)} className='w-full pointer-events-auto mb-2 gap-2 mx-1 flex items-center justify-center py-2 bg-white text-black font-bold rounded-md'>
         <BsBag /> Add to cart
        </button>
        <button onClick={handleLike} className='py-2 px-2 mb-2 pointer-events-auto rounded-md me-2 bg-white'>
