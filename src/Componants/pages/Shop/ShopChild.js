...
function ShopChild({sho, setIsOpen, setCartCondi, cartCondi}) {
  ...
  <div className='font-thin my-2'>
    <div className='flex justify-between'>
      <p className='text-gray-400 truncate'>{category}</p>
      <p>{rating}<span className='text-orange-400'>★</span></p>
    </div>
    <p className={`font-bold my-2 truncate ${them?'text-gray-500' : 'text-gray-300'}`}>{name}</p>
    <p className={`font-bold ${them? 'text-gray-600' : 'text-gray-200'}`}>${price}</p>
    <p className='text-gray-600 truncate'>{sizes.length} sizes available</p>
  </div>
</div>
  )
}
